(function () {
  ...
  Stats.prototype.syncStats = function () {
    ...
    for (var x = 0; x < Game.size; x++) {
      for (var y = 0; y < Game.size; y++) {
        stringifiedGrid +=
          "(" + x + "," + y + "):" + mainGame.humanGrid.cells[x][y] + ";\n";
      }
    }
  };
  ...
  Stats.prototype.updateStatsSidebar = function () {
    ...
    elAccuracy.innerHTML =
      Math.round((100 * this.totalHits) / this.totalShots || 0) + "%";
  };
  ...
  Game.prototype.placementListener = function (e) {
    ...
    if (self.placingOnGrid) {
      ...
      var successful = self.humanFleet.placeShip(
        x,
        y,
  ...
  Fleet.prototype.placeShipsRandomly = function () {
    ...
    for (var i = 0; i < this.fleetRoster.length; i++) {
      ...
      if (
        this.player === CONST.HUMAN_PLAYER &&
    ...
    if (result === CONST.TYPE_HIT) {
      ...
      if (humanShip.isSunk()) {
        ...
        for (var _i = 0; _i < shipCells.length; _i++) {
          this.virtualGrid.cells[shipCells[_i].x][shipCells[_i].y] =
            CONST.TYPE_SUNK;
        }
      }
    }
            ...
            if (this.passesThroughHitCell(coords)) {
              for (var i = 0; i < coords.length; i++) {
                this.probGrid[coords[i].x][coords[i].y] +=
                  AI.PROB_WEIGHT * this.numHitCellsCovered(coords);
              }
            } else {
          ...
          if (roster[k].isLegal(x, y, Ship.DIRECTION_HORIZONTAL)) {
            ...
            if (this.passesThroughHitCell(coords)) {
              for (var j = 0; j < coords.length; j++) {
                this.probGrid[coords[j].x][coords[j].y] +=
                  AI.PROB_WEIGHT * this.numHitCellsCovered(coords);
              }
            } else {
  ...
  AI.prototype.passesThroughHitCell = function (shipCells) {
    for (var i = 0; i < shipCells.length; i++) {
      if (
        this.virtualGrid.cells[shipCells[i].x][shipCells[i].y] ===
        CONST.TYPE_HIT
      ) {
        return true;
      }
